...
tickerDF = {}
drawdownCutoff = {}
drawdownCutoff['TLT'] = -.05
drawdownCutoff['IEF'] = -.04
drawdownCutoff['SPY'] = -.05
drawdownCutoff['OIL'] = -.05
drawdownCutoff['TOTL'] = -.04
drawdownCutoff['BSV'] = -.04
drawdownCutoff['XLU'] = -.04
drawdownCutoff['XLP'] = -.04
drawdownCutoff['DEF'] = -.04
...
plt.title(plotTicker + ' 5/15/30-day (orange,purple,blue) Trailing Vol vs. 15-day Fwd. Return')
sns.regplot(x=inputTable.loc[:,'vol30'], y=inputTable.loc[:,'fwd15'],order=2, marker='o', scatter_kws={'s':1})
sns.regplot(x=inputTable.loc[:,'vol15'], y=inputTable.loc[:,'fwd15'],order=2, marker='o', scatter_kws={'s':1})
sns.regplot(x=inputTable.loc[:,'vol5'], y=inputTable.loc[:,'fwd15'],order=2, marker='o', scatter_kws={'s':1})

plt.figure(figsize=(8,8), dpi=200)
plt.title(plotTicker + ' 5/15/30-day (orange,purple,blue) Trailing Vol vs. Drawdown Trigger')
sns.regplot(x=inputTable.loc[:,'vol30'], y=inputTable.loc[:,'drawdown'],order=2, marker='o', scatter_kws={'s':1})
sns.regplot(x=inputTable.loc[:,'vol15'], y=inputTable.loc[:,'drawdown'],order=2, marker='o', color='purple', scatter_kws={'s':1})
sns.regplot(x=inputTable.loc[:,'vol5'], y=inputTable.loc[:,'drawdown'],order=2, marker='o', color='orange', scatter_kws={'s':1})
